...
4. Run `git diff --cached` (with no extra flags such as `--stat` that would alter the output format) to review all staged changes and verify that every change belongs in this commit. If anything irrelevant is staged, stop and tell the user.
